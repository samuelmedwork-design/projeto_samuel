...
import {
  FiPlus, FiTrash2, FiChevronDown, FiChevronRight, FiArrowLeft,
  FiCheck, FiX, FiClipboard, FiUsers, FiAlertTriangle,
} from "react-icons/fi";
...
export default function GsePage() {
  ...
  const {
    companies, sectors, positions, assessments,
  ...
  // Cargos que já estão em algum GSE desta empresa
  ...
  const toggleGse = (gseId: string, numero: number) => {
    ...
    <div className="p-8 max-w-5xl">
      ...
      {companyGses.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          <FiUsers size={40} className="text-slate-300 mx-auto mb-4" />
          <p className="text-slate-500">Nenhum GSE cadastrado.</p>
          <p className="text-slate-400 text-sm mt-1">Clique em "Adicionar GSE" para criar o primeiro grupo.</p>
        </div>
      )}
      ...
      <div className="space-y-3">
        {companyGses.map((gse) => {
          ...
          // Avaliações disponíveis para este GSE: as que pertencem a cargos do GSE
          const gsePositionObjs = positions.filter((p) => gse.positionIds.includes(p.id));
          const availableAssessments = assessments.filter((a) => gse.positionIds.includes(a.positionId));

          return (
            <div key={gse.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              {/* Header row */}
              <button
                onClick={() => toggleGse(gse.id, gse.numero)}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
                      {gse.positionIds.length} cargo(s)
                    </span>
                    <span className="text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
                      {gse.risks.length} risco(s)
                    </span>
                    <span className="text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
                      {gse.assessmentIds.length} avaliação(ões)
                    </span>
                    {gse.conclusao && (
                      <span className="text-xs text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded">Conclusão preenchida</span>
                    )}
                  </div>
                </div>
                <button
                ...
                {isExpanded ? <FiChevronDown size={18} className="text-slate-400 shrink-0" /> : <FiChevronRight size={18} className="text-slate-400 shrink-0" />}
              </button>
                ...
                <div className="border-t border-slate-100">
                  ...
                  <div className="flex gap-1 px-4 pt-3 bg-slate-50 border-b border-slate-200">
                    {(["cargos", "riscos", "avaliacoes", "conclusao"] as const).map((t) => (
                      <button key={t} onClick={() => setActiveTab((prev) => ({ ...prev, [gse.id]: t }))} className={tabCls(gse.id, t)}>
                  ...
                  <div className="p-6">
                    ...
                    {tab === "cargos" && (
                      <div>
                        <p className="text-sm text-slate-500 mb-4">
                          Selecione os cargos que pertencem a este GSE. Um cargo só pode pertencer a um GSE por vez.
                        </p>
                                      ...
                                      return (
                                        ...
                                        >
                                          <span className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 ${isLinked ? "bg-emerald-600 border-emerald-600" : "border-slate-300"}`}>
                                            {isLinked && <FiCheck size={12} className="text-white" />}
                                          </span>
                                          <div className="min-w-0">
                                            <p className="font-medium truncate">{pos.name}</p>
                                            {isUsedElsewhere && (
                                              <p className="text-xs text-slate-400">Em outro GSE</p>
                                            )}
                                          </div>
                                        </button>
                                      );
                    ...
                    {tab === "riscos" && (
                      <div>
                        <p className="text-sm text-slate-500 mb-4">
                          Selecione os riscos ergonômicos evidenciados para este grupo ({gse.risks.length} selecionado(s)).
                        </p>
                          ...
                          {GSE_RISKS.map((risco) => {
                            ...
                            return (
                              <button
                                ...
                                className={`flex items-start gap-3 px-4 py-3 rounded-lg border text-left text-sm transition-colors ${
                                  isLinked
                    ...
                    {tab === "avaliacoes" && (
                      <div>
                        <p className="text-sm text-slate-500 mb-4">
                          Vincule avaliações (checklists) realizadas em cargos deste GSE.
                          {gse.positionIds.length === 0 && <span className="text-amber-600"> Adicione cargos ao GSE primeiro.</span>}
                        </p>
                        {availableAssessments.length === 0 ? (
                          <div className="text-center py-8 text-slate-400">
                            <FiClipboard size={32} className="mx-auto mb-3 text-slate-300" />
                            <p className="text-sm">Nenhuma avaliação encontrada para os cargos deste GSE.</p>
                          </div>
                        ) : (
                            ...
                            {availableAssessments.map((assessment) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full flex items-start gap-3 px-4 py-3 rounded-lg border text-left text-sm transition-colors ${
                                    isLinked
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-slate-500 mt-0.5">
                                      {pos?.name || assessment.positionId} · {sec?.name || ""} · {assessment.workstation}
                                    </p>
                                    <p className="text-xs text-slate-400 mt-0.5">
                                      Observado: {assessment.observedWorker} · {new Date(assessment.createdAt).toLocaleDateString("pt-BR")}
                                    </p>
                                  </div>
                                  {ncCount > 0 && (
                                    <span className="flex items-center gap-1 text-xs text-red-600 bg-red-50 px-2 py-0.5 rounded shrink-0">
                                      <FiAlertTriangle size={11} /> {ncCount} NC
                                    </span>
                                  )}
                                </button>
                              );
                    ...
                    {tab === "conclusao" && (
                      <div>
                        <p className="text-sm text-slate-500 mb-3">
                          Registre a análise ergonômica e conclusão do avaliador para este grupo.
                        </p>
                        <textarea
